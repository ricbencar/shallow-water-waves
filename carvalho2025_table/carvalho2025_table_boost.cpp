/***********************************************************************
 * Program: carvalho2025_table_boost.cpp
 *
 ...
 *
 * Output:
 * The program generates a formatted text file named "carvalho2025_table_boost.txt". This file
 ...
 *
 *
 * Boost.Math Library - Installation and Compilation Instructions for Windows (MinGW/g++):
 *
 * If you are using g++ (e.g., from MinGW-w64) on Windows, you cannot use Boost libraries
 * pre-built for MSVC. You need to build Boost from source specifically for your g++ compiler.
 *
 * Step 1: Download Boost Source Code
 * 1. Go to the official Boost website: https://www.boost.org/
 * 2. Download the **source code** for Boost (e.g., boost_1_88_0.zip).
 * 3. Extract the downloaded archive to a directory, for example:
 * C:\local\boost_1_88_0_source
 *
 * Step 2: Open MinGW-w64 Command Prompt
 * 1. Ensure your system's PATH environment variable includes the MinGW-w64 `bin` directory
 * (e.g., C:\MinGW\bin) so that `g++`, `ar`, and other build tools are accessible from any command prompt.
 * Alternatively, open the specific MinGW-w64 shell if your installation provides one.
 *
 * Step 3: Build Boost
 * 1. Navigate to the extracted Boost source directory in your command prompt:
 * cd C:\local\boost_1_88_0_source
 * 2. Run the bootstrap script to prepare the build system. For MinGW, use `bootstrap.bat mingw`:
 * .\bootstrap.bat mingw
 * 3. Now, build the Boost libraries. This step can take a significant amount of time
 * (tens of minutes to hours, depending on your system and CPU).
 * .\b2.exe --toolset=gcc --build-type=complete --with-math --with-system --with-chrono --with-thread address-model=64 install --prefix=C:\local\boost_1_88_0_mingw
 * - `--toolset=gcc`: Specifies to use the GCC toolset (MinGW's g++).
 * - `--build-type=complete`: Builds both static and shared libraries.
 * - `--with-math --with-system --with-chrono --with-thread`: Explicitly builds only the libraries needed for this project and common dependencies. You can customize this list.
 * - `address-model=64`: Builds for 64-bit architecture.
 * - `install --prefix=C:\local\boost_1_88_0_mingw`: Installs the compiled headers and libraries to this new directory.
 *
 * Step 4: Verify Boost Installation
 * 1. After the build completes, verify that the headers are in `C:\local\boost_1_88_0_mingw\include\boost-1_88`
 * and the compiled libraries (`.a` files) are in `C:\local\boost_1_88_0_mingw\lib`.
 * You can use `dir C:\local\boost_1_88_0_mingw\lib` to see the exact library filenames.
 * Look for files like `libboost_math_c99-mgw14-mt-x64-1_88.a` and `libboost_system-mgw14-mt-x64-1_88.a`.
 *
 * Compilation Instructions for carvalho2025_table_boost.cpp (using MinGW/g++ with custom Boost build):
 *
 * To compile this program with high optimization levels and the Boost.Math library,
 * use the following g++ command. Ensure the paths match your Boost installation
 * from Step 3, and the library names match the exact filenames found in Step 4.
 *
 * g++ -O3 -march=native -std=c++17 -Wall -Wextra -pedantic -Wconversion -Wsign-conversion -IC:/local/boost_1_88_0_mingw/include/boost-1_88 -LC:/local/boost_1_88_0_mingw/lib -o carvalho2025_table_boost.exe carvalho2025_table_boost.cpp -lboost_math_c99-mgw14-mt-x64-1_88 -lboost_system-mgw14-mt-x64-1_88
 *
 * - `-O3`: Enables aggressive optimization for speed.
 * - `-march=native`: Optimizes the compiled code for the specific CPU architecture of your machine.
 * - `-std=c++17`: Specifies that the code should be compiled according to the C++17 standard.
 * - `-Wall -Wextra -pedantic`: Activates a comprehensive set of warning messages to help identify potential issues and enforce stricter adherence to the C++ standard.
 * - `-Wconversion -Wsign-conversion`: Specific warnings for implicit type conversions that might lead to data loss or unexpected behavior.
 * - `-IC:/local/boost_1_88_0_mingw/include/boost-1_88`: Tells the compiler to look for Boost header files in this directory. Adjust if your Boost headers are in a different subdirectory (e.g., just `boost` directly under `include`).
 * - `-LC:/local/boost_1_88_0_mingw/lib`: Tells the linker to look for Boost compiled library files (`.a` files) in this directory.
 * - `-o carvalho2025_table_boost.exe`: Specifies the name of the output executable file.
 * - `carvalho2025_table_boost.cpp`: The name of your source code file.
 * - `-lboost_math_c99-mgw14-mt-x64-1_88`: Links your program against the Boost.Math library's C99 special functions component. This exact name is crucial and must match the `.a` file found in your `lib` directory (e.g., `libboost_math_c99-mgw14-mt-x64-1_88.a`).
 * - `-lboost_system-mgw14-mt-x64-1_88`: Links against the Boost.System library, which is often a dependency for other Boost components. Again, the exact name is crucial.
 *
 ***********************************************************************/
...
#include <cmath>        // Contains a wide range of mathematical functions (e.g., `std::sqrt` for square root, `std::log` for natural logarithm, `std::pow` for exponentiation).
...
#include <stdexcept>    // For standard exception types like `std::invalid_argument`, `std::domain_error`, and `std::runtime_error`.

// Suppress specific warnings from Boost headers.
// These pragmas push the current warning state, ignore specific warnings,
// include the Boost headers, and then pop the warning state back.
// This is a common practice to avoid warnings from third-party libraries
// that you cannot modify.
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wsign-conversion"

// Boost.Math Library Includes:
// These headers provide access to Boost's highly accurate and robust special functions,
// including the incomplete gamma functions.
#include <boost/math/special_functions/gamma.hpp> // For boost::math::gamma_p, boost::math::gamma_q, boost::math::tgamma (unnormalized incomplete gamma)
#include <boost/math/policies/error_handling.hpp> // For configuring Boost's error handling policies (though default policies often suffice)
...
// Declares functions for wave height calculations.
...
// Declares functions for the system of non-linear equations.
double F1(double H1_Hrms, double H2_Hrms, double Htr_Hrms);
double F2(double H1_Hrms, double H2_Hrms, double Htr_Hrms);

// Declares function to solve the 2x2 linear system Ax = b using Cramer's rule (or simple substitution)
...
// --- START OF GAMMA FUNCTIONS IMPLEMENTATION (USING BOOST.MATH) ---

/**
 * @brief Computes the normalized lower incomplete gamma function P(a, x) = γ(a, x)/Γ(a)
 * using Boost.Math library.
 *
 * Boost.Math's `gamma_p` function is a highly optimized and numerically stable
 * implementation of the regularized lower incomplete gamma function. It handles
 * various input ranges and edge cases internally, providing robust results.
 *
 ...
 * @return The computed value of P(a, x).
 * @throws `std::domain_error` or other standard exceptions (e.g., `std::overflow_error`)
 * as per Boost.Math's default or configured error handling policies if input parameters are invalid
 * or if a numerical issue occurs.
 */
double incomplete_gamma_p(double a, double x) {
    // Boost.Math handles input validation and numerical stability internally.
    // It will throw standard exceptions (like std::domain_error) or return NaN/Inf
    // based on its configured policies.
 ...
 * using Boost.Math library.
 *
 * Boost.Math's `gamma_q` function is a highly optimized and numerically stable
 * implementation of the regularized upper incomplete gamma function. It is designed
 * to avoid catastrophic cancellation that can occur when computing `1 - P(a,x)`
 * for cases where `P(a,x)` is very close to 1.
 *
 * @param a The 'a' parameter of the incomplete gamma function. Must be positive.
 * @param x The 'x' parameter of the incomplete gamma function. Must be non-negative.
 ...
 * using Boost.Math library.
 *
 * Boost.Math's `tgamma(a, x)` overload directly computes the unnormalized
 * upper incomplete gamma function. This is a robust implementation that
 * handles potential overflow/underflow internally.
 *
 * @param a The 'a' parameter (shape parameter) of the incomplete gamma function. Must be positive.
 ...
 * @throws `std::invalid_argument` (if `a <= 0` or `x < 0` are explicitly checked before calling Boost.Math)
 * or `std::domain_error`/`std::overflow_error` as per Boost.Math policies if numerical issues arise.
 */
double incomplete_gamma(double a, double x) {
    // Boost.Math's tgamma(a, x) directly computes the unnormalized upper incomplete gamma function.
    // It handles input validation and numerical stability internally.
    // Explicit checks for a <= 0.0 or x < 0.0 are often redundant if Boost.Math's default
    // domain error policy (throwing std::domain_error) is sufficient.
    return boost::math::tgamma(a, x);
}

// --- END OF GAMMA FUNCTIONS IMPLEMENTATION (USING BOOST.MATH) ---
 ...
 * @throws `std::domain_error` if underlying gamma functions encounter invalid domains.
 * @throws `std::runtime_error` if other numerical issues occur.
 */
double calculate_H1N(double N_val, double H1, double H2, double k1, double k2, double Htr) {
    // Input validation: Ensures all parameters are within their mathematically valid ranges.
    if (H1 <= 0.0 || H2 <= 0.0) {
...
int main()
{
    // Open an output file named "carvalho2025_table_boost.txt" for writing.
    // `std::ofstream` is used to create an output file stream.
    ofstream fout("carvalho2025_table_boost.txt");
    // Check if the file was successfully opened. If `fout` is in a bad state (e.g., due to permissions or disk full),
    // it evaluates to `false`.
    if (!fout) {
        cerr << "Error opening output file: carvalho2025_table_boost.txt" << endl; // Print an error message to console.
    ...
    fout << setw(9) << "Htr/Hrms"
         ...
         << setw(11) << "H1/100/Hrms"  // Normalized mean of the highest 1/100th waves (often H1%).
    ...
    for (double Htr_Hrms = Htr_Hrms_start; Htr_Hrms <= Htr_Hrms_end + EPSILON; Htr_Hrms += Htr_Hrms_step) {
        try { // Begin a try-catch block to gracefully handle potential runtime errors during calculations.
            ...
            fout << setw(11) << h1n_values[0]  // H1/3/Hrms
                 ...
                 << setw(11) << h1n_values[3]  // H1/100/Hrms
                 << setw(12) << h1n_values[4]  // H1/250/Hrms
                 << setw(13) << h1n_values[5]  // H1/1000/Hrms
                 << endl; // Move to the next line in the output file.

        } catch (const domain_error& e) { // Catch `std::domain_error` exceptions (from gamma functions).
            cerr << "Domain error for Htr_Hrms = " << Htr_Hrms << ": " << e.what() << endl;
            fout << setw(9) << Htr_Hrms
                 << setw(9) << "ERROR"
                 << setw(9) << "ERROR"
                 << setw(11) << "ERROR"
            ...
            // These typically indicate issues during the execution, such as solver failure to converge or gamma function convergence failure.
    ...
    cout << "Calculation complete. Results saved to carvalho2025_table_boost.txt" << endl; // Inform the user via console.
