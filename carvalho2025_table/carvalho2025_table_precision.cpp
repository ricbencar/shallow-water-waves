 ...
 *
 * Output:
 * The program generates a formatted text file named "carvalho2025_table.csv". This file
...
void get_initial_guesses(long double Htr_Hrms, long double &H1_initial, long double &H2_initial) {
    ...
    long double logHtr = log(Htr_Hrms);
    H1_initial = 1.0L + exp(-1.705L - 2.329L * logHtr - 0.313L * logHtr * logHtr);
 ...
 * computed results to a formatted CSV file.
 *
 * @return 0 if the program executes successfully, 1 if there is an error (e.g., file opening failure).
 */
int main()
{
    // Open an output file named "carvalho2025_table.csv" for writing.
    ofstream fout("carvalho2025_table.csv");
    // Check if the file was successfully opened. If `fout` is in a bad state (e.g., due to permissions or disk full),
    // it evaluates to `false`.
    if (!fout) {
        cerr << "Error opening output file: carvalho2025_table.csv" << endl; // Print an error message to console.
    ...
    // Write the header row to the output file in CSV format.
    fout << "Htr/Hrms,H1/Hrms,H2/Hrms,H1/3/Hrms,H1/10/Hrms,H1/50/Hrms,H1/100/Hrms,H1/250/Hrms,H1/1000/Hrms" << endl;
    ...
    for (long double Htr_Hrms = Htr_Hrms_start; Htr_Hrms <= Htr_Hrms_end + EPSILON; Htr_Hrms += Htr_Hrms_step) {
        try { // Begin a try-catch block to gracefully handle potential runtime errors during calculations.
            ...
            // Set output formatting for the current row in the file for maximum precision.
            fout << fixed << setprecision(numeric_limits<long double>::digits10 + 1); // Use max digits for long double
            ...
            // Output each calculated H1/N value from the vector to the file, separated by commas.
            for (long double val : h1n_values) {
                fout << "," << val;
            }
            ...
            fout << Htr_Hrms << ",ERROR,ERROR,ERROR,ERROR,ERROR,ERROR,ERROR,ERROR" << endl;
    ...
    cout << "Calculation complete. Results saved to carvalho2025_table.csv" << endl; // Inform the user via console.
