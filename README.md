...
5.  **CWD Solution**: If $\tilde{H}\_{tr} ≤ 2.75$, the program proceeds to solve the system of non-linear equations derived from the CWD to find the dimensionless scale parameters $\tilde{H}\_1$ and $\tilde{H}\_2$. From these, it computes the required statistical wave height ratios, e.g., $H\_{1/3}$, $H\_{1/10}$, $H\_{1/50}$, $H\_{1/100}$, etc.
