...
* **Key Parameter Calculation**: Computes mean square wave height ($H_{rms}$), and dimensional/dimensionless transitional wave heights ($H_{tr\_dim}$, $\tilde{H}_{tr}$) from the free-surface variance ($m_0$).
...
* **Complete Gamma Function ($\Gamma(z)$):**
    ```math
    \Gamma(a) = \int_0^{\infty} t^{a-1} e^{-t} dt \quad (a > 0)
    ```
* **Unnormalized Lower Incomplete Gamma Function ($\gamma(a,x)$):**
    ```math
